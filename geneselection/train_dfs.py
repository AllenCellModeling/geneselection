...
def run(
    ...
    network = getattr(network_module, network_name)(
        n_in=anndata.shape[1], **network_kwargs["kwargs"]
    )
    ...
    opt = getattr(optim_module, optim_name)(
        [
            {"params": network.main.parameters()},
            {"params": network.w, "lr": optim_kwargs["lr_w"]},
        ],
        **optim_kwargs["kwargs"]
    )
